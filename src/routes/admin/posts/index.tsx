...
function RouteComponent() {
  ...
  const { data: postsStats } = useQuery({
    queryKey: ['get-posts-stats', fromDate, toDate],
    queryFn: () =>
      getPostsStats({
  ...
  const chartData = useMemo(() => {
    if (!postsStats) return []

    return Object.keys(postsStats.wisePosts).flatMap((year) => {
      const yearData = postsStats.wisePosts[year]
  ...
  return (
    <>
      ...
      <AdminLayout>
        ...
        >
          <Text fw={500}>Posts Statistics</Text>
          <Group mt={16}>
            <DatePickerInput
              value={fromDate}
